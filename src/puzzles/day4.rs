...
impl Card {
    fn matches(&self) -> u32 {
        return self.scratched.iter().fold(0, |acc, it| acc + (self.winning.contains(it) as u32));
    }
...
fn parse_game(input: &str) -> Vec<Card> {
    ...
    while tokens.peek().is_some() {
        ...
        let winning: Vec<i32> = tokens.next().unwrap().into_inner().map(|it| it.as_str().parse::<i32>().unwrap()).collect();
        let scratched: Vec<i32> = tokens.next().unwrap().into_inner().map(|it| it.as_str().parse::<i32>().unwrap()).collect();
        cards.push(Card {id: card_id, scratched: scratched, winning: winning });
    }
    return cards;
}
...
pub fn p1(input: String) {
    ...
    for card in cards.iter() {
        println!("{:?} {:?} | {:?} => {:?}", card.id, card.winning, card.scratched, card.points());
    }
...
fn resolve_scratchcard(card: &Card, following_card_stock: &[Card]) -> u32 {
    ...
    for m in 1..matches+1 {
        ...
        won_cards += resolve_scratchcard(&following_card_stock[index-1], &following_card_stock[index..])
    }
